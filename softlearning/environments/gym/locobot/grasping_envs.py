...
class ImageLocobotMultistepGraspingEnv(LocobotBaseEnv):
    ...
    def __init__(self, min_blocks=0, max_blocks=6, min_other_blocks=0, max_other_blocks=6, 
        ...
        defaults["observation_space"] = spaces.Dict({
            "current_ee": spaces.Box(low=-1.0, high=1.0, shape=(2,)),
            # "pixels": added by PixelObservationWrapper
        })
        defaults["action_dim"] = 5
        ...
        if crop_output:
            ...
            defaults["image_size"] = locobot_interface.IMAGE_SIZE
            defaults['camera_loop_pos'] = np.array([-0.14593288110963892, 1.113989290515928, 0.07817330139840732])#
        defaults.update(params)
    ...
    def get_observation(self):
        ...
        ee_local = self.interface.get_ee_local()
    ...
    def step(self, a):
        self.interface.apply_action(self.denormalize_action(a))
